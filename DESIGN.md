...
Core analogy: like Google MediaPipe processes camera frames to recognize hand gestures in real-time, ScreenPilot processes screen frames to recognize UI elements in real-time.
...
ScreenPilot's core advantage over Anthropic Computer Use, OmniParser, and similar general-purpose tools: **deep understanding of a specific application + high-precision operation**.
...
ScreenPilot should not assume one global detector for all software. Each target application should ship as an independent runtime pack that can be loaded, unloaded, and upgraded without affecting other applications.
...
### 1. Screen Capture (`screenpilot/capture/screen_capture.py`)
...
### 2. Change Detector (`screenpilot/capture/change_detector.py`)
...
### 3. UI Detector (`screenpilot/detection/detector.py`)
...
### 3.5. App Router + Model Registry (`screenpilot/runtime/`)
...
### 4. Element Tracker (`screenpilot/tracker/element_tracker.py`)
...
### 5. Cursor Monitor (`screenpilot/cursor/cursor_monitor.py`)
...
### 6. Action Executor (`screenpilot/actions/executor.py`)
...
### 7. LLM Interface (`screenpilot/llm/`)
...
### 8. Training Pipeline (`screenpilot/training/`)
...
### 10. Screen Classifier + Element Verifier (`screenpilot/detection/`)
