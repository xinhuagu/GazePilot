...
ScreenPilot is a real-time screen monitoring and automation tool. It uses a custom-trained YOLO neural network to detect UI elements (buttons, menus, input fields, etc.) in a VDI-hosted Windows application from screen pixels alone, enabling LLM-driven precise software operation.
...
ruff check screenpilot/ && ruff format screenpilot/
